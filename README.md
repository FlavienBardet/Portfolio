<div align="center">

# Flavien Bardet | Behavioral Data Strategist

> **"Je ne vois pas des points dans un graphique, je vois des motivations, des freins et des parcours de vie."**

[![LinkedIn](https://img.shields.io/badge/-LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/flavien-bardet-38649a18b/)
[![Email](https://img.shields.io/badge/-Email-EA4335?style=for-the-badge&logo=gmail&logoColor=white)](mailto:issadun@gmail.com)
[![Portfolio](https://img.shields.io/badge/-Portfolio-10B981?style=for-the-badge&logo=vercel&logoColor=white)](https://flavienbardet.com)

</div>

---

## 🧠 À Propos : De la Psychologie aux Algorithmes

Je ne vois pas des points dans un graphique, **je vois des motivations, des freins et des parcours de vie**. Mon rôle est de décoder ces histoires humaines pour éclairer la stratégie business.

Pendant 5 ans, j'ai accompagné des patients en psychothérapie clinique, apprenant à écouter au-delà des mots, à détecter les schémas invisibles et à poser des diagnostics qui changent des vies. Aujourd'hui, je transpose cette rigueur scientifique vers la **Data Analysis**.

> *"La donnée n'est rien sans le contexte humain qui la génère. Un churn rate, c'est quelqu'un qui est parti. Un pic de ventes, c'est une émotion qui a déclenché un achat. Ma mission : comprendre le pourquoi derrière le combien."*

Mon profil hybride combine l'**empathie du praticien** avec la **puissance technique** (Python, SQL, Power BI) pour transformer des chiffres en récits stratégiques qui résonnent avec les décideurs.

---

## 🔬 The Psychologist's Edge : Ce Qui Distingue Mon Approche

| | Compétence | Application Data |
|:--|:-----------|:-----------------|
| 🧩 | **Détection des biais cognitifs** | Identifier le "regret d'achat" Black Friday, le "36-Month Cliff" de fidélité, les patterns d'abandon invisibles dans les datasets |
| 🔍 | **Rigueur de la méthode clinique** | Hypothèse → Test → Validation. Je ne fais jamais "dire" aux chiffres ce qu'ils ne disent pas. Vérification des P-values, audit anti-hallucination |
| 🛡️ | **Éthique centrée sur le sujet** | Secret médical → RGPD. L'anonymisation n'est pas une contrainte, c'est une évidence. La donnée appartient à l'humain qui l'a générée |

---

## 🛠️ Expertise Technique

| Domaine | Compétences |
|:--------|:------------|
| **Collecte & Gouvernance** | SQL avancé, Nettoyage Python (Pandas), Conformité RGPD |
| **Analyse & Prédiction** | Machine Learning (Scikit-Learn), Statistiques descriptives, A/B Testing |
| **Visualisation & Stratégie** | Power BI, Storytelling, Architecture de Dashboard |

```
📦 Data Collection      → SQL, Power Query, APIs
🔬 Data Processing      → Python (Pandas, NumPy), ETL
🤖 Machine Learning     → Scikit-Learn, Pipelines, Régression
📈 Visualization        → Power BI, Matplotlib, Seaborn
🔐 Governance           → RGPD, Data Quality, Documentation
```

---

## 🚀 Référentiel de Projets (11)

<table>
<thead>
<tr>
<th align="center">#</th>
<th align="left">Projet</th>
<th align="left">Stack</th>
<th align="left">📊 Description d'Impact</th>
</tr>
</thead>
<tbody>
<tr>
<td align="center">🚀</td>
<td><strong>P11 | Portfolio Bio-Digital</strong></td>
<td><code>HTML</code> <code>Tailwind</code> <code>JS</code></td>
<td>Conception d'un écosystème portfolio complet certifiant la capacité à piloter des solutions Data de bout en bout pour l'aéronautique.</td>
</tr>
<tr>
<td align="center">🎮</td>
<td><strong><a href="./FB_P10_analysez-une-demande-business-et-identifiez-les-segments-du-marche-les-plus-pertinents-pour-votre-client_2025-12-03T151416%20(1)">P10 | Project Vanguard</a></strong></td>
<td><code>Power BI</code></td>
<td>Analyse Go-To-Market sécurisant un investissement de <strong>20M€</strong> en identifiant les niches de joueurs non-toxiques.</td>
</tr>
<tr>
<td align="center">📦</td>
<td><strong><a href="./FB_P9_ameliorez-les-performances-de-lentreprise_2025-10-17T120621">P9 | Bottleneck : Architecture BI</a></strong></td>
<td><code>Power BI</code> <code>DAX</code></td>
<td>Optimisation de la trésorerie via un schéma en étoile, identifiant <strong>15% de stock dormant</strong> pour maximiser la marge.</td>
</tr>
<tr>
<td align="center">🏙️</td>
<td><strong><a href="./FB_P8_analysez-levolution-des-prix-de-limmobilier-avec-python-1_2025-08-01T124629">P8 | Valuation ML</a></strong></td>
<td><code>Python</code> <code>Sklearn</code></td>
<td>Algorithme prédictif révélant <strong>38% de valeur latente</strong> dans un portefeuille immobilier via Machine Learning.</td>
</tr>
<tr>
<td align="center">🏥</td>
<td><strong><a href="./FB_P7_creez-un-tableau-de-bord-dynamique-avec-powerbi-pour-visualiser-lavancement-de-projets-1_2025-06-14T192232%20(1)">P7 | Sanitoral : PMO</a></strong></td>
<td><code>Power BI</code></td>
<td>Automatisation du pilotage de projet via une approche Produit, supprimant <strong>4h de reporting manuel</strong> par manager.</td>
</tr>
<tr>
<td align="center">🕵️</td>
<td><strong><a href="./FB_P6_optimisez-la-gestion-nettoyez-les-donnees-du-stock-dune-boutique_2025-05-02T171256">P6 | Bottleneck : Data Audit</a></strong></td>
<td><code>Python</code> <code>Pandas</code></td>
<td>Audit de qualité réconciliant les flux ERP/Web et identifiant <strong>111 produits orphelins</strong> pour fiabiliser le CA.</td>
</tr>
<tr>
<td align="center">🛒</td>
<td><strong><a href="./FB_P5_manipulez-une-base-de-donnees-avec-sql-pour-suivre-la-satisfaction-client_2025-03-28T175014">P5 | RetailInsight360</a></strong></td>
<td><code>SQL</code> <code>MariaDB</code></td>
<td>Audit SQL de 100k+ transactions prouvant l'impact financier des <strong>biais cognitifs</strong> (regret d'achat) lors du Black Friday.</td>
</tr>
<tr>
<td align="center">🛡️</td>
<td><strong><a href="./FB_P4_collectez-des-donnees-en-respectant-les-normes-rgpd-1_2024-11-18T171649">P4 | Dev'Immediat : Privacy</a></strong></td>
<td><code>Power Query</code></td>
<td>Mise en conformité RGPD d'un CRM via un pipeline d'anonymisation ETL, <strong>levant les sanctions CNIL</strong> pour sécuriser l'activité.</td>
</tr>
<tr>
<td align="center">🗃️</td>
<td><strong><a href="./FB_P3_requetez-une-base-de-donnees-avec-sql-1_2024-11-04T102040">P3 | SQL Foundations</a></strong></td>
<td><code>SQL</code></td>
<td>Construction et requêtage d'une base de données relationnelle complexe pour automatiser l'extraction d'indicateurs clés.</td>
</tr>
<tr>
<td align="center">📉</td>
<td><strong><a href="./FB_P2_visualisez-des-donnees-avec-excel-1_2024-10-11T150908">P2 | Strategic Churn Analysis</a></strong></td>
<td><code>Excel</code></td>
<td>Analyse comportementale identifiant le <strong>"36-Month Cliff"</strong> pour cibler 0.5% des clients à haut risque et optimiser la rétention.</td>
</tr>
<tr>
<td align="center">🎯</td>
<td><strong>P1 | Roadmap Stratégique</strong></td>
<td><code>Méthodologie</code></td>
<td>Définition de la trajectoire de montée en compétences et immersion dans les fondamentaux de l'analyse de données.</td>
</tr>
</tbody>
</table>

---

## 🎯 Secteurs d'Expertise

| Secteur | Pertinence |
|:--------|:-----------|
| **🏥 MedTech / e-Santé** | 5 ans d'expérience clinique. Je parle le langage médical et comprends les parcours patients. |
| **✈️ Aéronautique** | Projet Aéroworld : modernisation Data d'une structure de 50 ans (RAG, Gouvernance, Maintenance Prédictive). |
| **🎓 EdTech** | Expertise des processus d'apprentissage et de cognition appliquée à l'UX. |
| **🏦 Fintech** | Analyse comportementale de l'attrition bancaire, modélisation du risque client. |

---

<div align="center">

### 💬 *"Chaque dataset raconte une histoire humaine. Mon rôle est de la décoder."*

**[Voir le Portfolio Complet →](https://flavienbardet.com)**

</div>
